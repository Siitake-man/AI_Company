...
## 7. 会議画面の状態・アクセシビリティ契約（2026-08-16追加）

会議画面と議事録画面では、見た目だけでなく「誰が確定した状態か」を表示上も区別する。

- LLMが生成する構造化議事録は `issues`、`proConTable`、`facts`、`openConcerns`、
  `aiRecommendation`、`memberAgreementLevels`、`nextActions` の7項目に限定する。
  `decisions` はLLM出力に含めず、ユーザーが編集・確定する入力として扱う。
- AI提言とユーザー決定事項は別領域で表示する。決定事項は空欄から編集でき、
  「入力内容を確定して保存」を実行してトランザクションが完了するまで学習済みとは表示しない。
- 主要操作は44px以上の操作領域を確保し、キーボード操作では `:focus-visible` を常に視認できるようにする。
  非同期の保存・エラー・完了状態は `role="status"` などのライブ領域で通知し、エラー時は入力へフォーカスを戻す。
- 800×600程度の画面では参加者・ホワイトボードを折りたたみ式にし、1100px以上では3ペインを使用する。
  横スクロールを発生させず、`prefers-reduced-motion: reduce` ではパルス、バウンス、スムーズスクロールを抑止する。
- 紙の質感はインラインdata URIではなく、管理対象のローカル資産 `src/assets/paper-noise.svg` を使用する。

これらの状態遷移は、レトロRPG風の穏やかな演出と両立させる。ただし業務画面に長いスクロール演出や
外部サイト由来のコード・素材・依存関係を持ち込まない。
