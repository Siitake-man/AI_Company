...
`src/lib/rag/vectorStore.ts` は `KnowledgeVectorStore` 契約と、テスト・オフライン開発用の `InMemoryKnowledgeVectorStore` を提供する。検索は必ず `project_id` で絞り込み、コサイン類似度と `topK` / `minScore` を適用する。さらに `VectorSearchOptions` の `roleCategory` / `departmentId` を任意フィルターとして適用し、同一プロジェクト内でもメンバーの役割・部署に応じた「違うレンズ」を維持する。LanceDBは同じ契約を実装する次の永続化adapterとして追加する。

## 次の実装単位

1. LanceDB adapter（`KnowledgeVectorStore` の永続実装）
2. ✅ `role_category` / `department_id` を含む検索契約の拡張（2026-08-31）
3. 会議保存後の取り込みhook
